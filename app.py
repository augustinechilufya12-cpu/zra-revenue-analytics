...
app.config['MODELS_FOLDER'] = r'C:\Users\AUGUSTINE\Documents\ZRA_Projcet\backend\trained_models'
...
# Configuration
app.config['DATA_FOLDER'] = os.path.join(os.path.dirname(__file__), 'data')
# In app.py, update the MODELS_FOLDER path:
app.config['MODELS_FOLDER'] = r'C:\Users\AUGUSTINE\Documents\ZRA_Projcet\backend\trained_models'
...
try:
    forecast_engine = AdvancedForecastEngine(app.config)
    if forecast_engine.models_loaded or forecast_engine.fallback_models:
        print("✅ Advanced Forecast Engine initialized successfully")
        print(f"📊 ML Models loaded: {len(forecast_engine.models)}")
        print(f"🔄 Fallback models: {len(forecast_engine.fallback_models)}")
    else:
        print("❌ Forecast Engine failed to initialize")
        forecast_engine = None
except Exception as e:
    print(f"❌ Failed to initialize Advanced Forecast Engine: {e}")
    forecast_engine = None
...
def health_check():
    ...
    ml_status = {
        ...
        'forecast_fallback_models': len(forecast_engine.fallback_models) if forecast_engine else 0,
        'forecast_engine_operational': (forecast_engine and 
                                      (forecast_engine.models_loaded or forecast_engine.fallback_models)) if forecast_engine else False
    }
...
def generate_forecast():
    ...
    try:
        ...
        forecast_data = {
            ...
            'generated_at': pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S'),
            'models_used': {
                'ml_models': list(forecast_engine.models.keys()),
                'fallback_models': list(forecast_engine.fallback_models.keys())
            }
        }
...
def get_forecast_model_status():
    ...
    try:
        ...
        return jsonify({
            'status': 'operational',
            'models_available': len(forecast_engine.models),
            'fallback_models_available': len(forecast_engine.fallback_models),
            'models_loaded': forecast_engine.models_loaded,
            'total_tax_types': len(model_details),
            'model_details': model_details
        })
...
# Anomaly Detection Routes (Keep these as they are separate functionality)
@app.route('/api/anomalies/detections')
def get_anomalies():
    """Get anomaly detection data"""
    try:
...
# Scenario Planning Routes (Keep these as they are separate functionality)
@app.route('/api/scenario/run-simulation', methods=['POST'])
def run_scenario_simulation():
    """Run scenario simulation using trained ML models"""
    try:
...
# Helper methods for scenario simulation (Keep these)
def generate_scenario_timeline(result, baseline_data):
    """Generate scenario timeline based on ML prediction"""
    baseline_values = baseline_data['Total_Revenue'].tolist()
    impact_factor = result['revenue_change'] / baseline_values[-1] if baseline_values[-1] != 0 else 0
...
if __name__ == '__main__':
    ...
    if forecast_engine:
        ...
        print(f"   - ML Models loaded: {len(forecast_engine.models)}")
        print(f"   - Fallback models: {len(forecast_engine.fallback_models)}")
        print(f"   - Operational: {forecast_engine.models_loaded or bool(forecast_engine.fallback_models)}")
        
        # Show which models are available
        if forecast_engine.models:
            print("   ✅ ML Models:")
            for model_name in forecast_engine.models.keys():
                print(f"      - {model_name}")
        
        if forecast_engine.fallback_models:
            print("   🔄 Fallback Models:")
            for model_name in forecast_engine.fallback_models.keys():
                print(f"      - {model_name}")
    else:
    ...
    # PRODUCTION SETTINGS - CHANGE THIS LINE ONLY:
